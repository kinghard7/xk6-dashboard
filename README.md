...
lang      | Language for the dashboard UI (default: `zh`, available: `zh` or `en`)
...
K6_WEB_DASHBOARD_LANG      | Language for the dashboard UI (default: `zh`, available: `zh` or `en`)
...
**Language Support**: The exported HTML report will automatically use the language specified by the `lang` parameter or `K6_WEB_DASHBOARD_LANG` environment variable. Users can also switch languages using the language toggle button in the report header.
...
**Language Support**: The CLI tool also supports the `--lang` flag for setting the initial language of generated reports and web UI:

```bash
# Generate report with Chinese UI (default)
k6-web-dashboard report events.ndjson report.html

# Generate report with English UI
k6-web-dashboard report --lang=en events.ndjson report.html

# Replay with Chinese UI (default)
k6-web-dashboard replay events.ndjson

# Replay with English UI
k6-web-dashboard replay --lang=en events.ndjson
```
